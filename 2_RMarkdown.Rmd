...
Note: knitting may require the installation of additional packages like `stringi` and `stringr`. If so, a window will pop up warning you of this requirement, and you can agree to install these as well. 
...
*   `error = TRUE` causes the render to continue even if code returns an error.
    This can be  useful if you need to debug exactly
...
```{r}
teeth_lost_per_kid = c(16, 16, 1) # Evee, Hazel, Nell
```
...
And here's the comic again but with a caption included, and the size reduced by 50% using the command:
...
Now try to knit your .Rmd file to a .pdf file. This can be done by changing the output line in the YAML header to `output: pdf_document`, or by selecting `Knit to PDF` from the dropdown menu under `Knit`. If you encounter issues knitting, make sure that you have LaTex properly installed on your computer. If you are on a Mac, make sure you also have XCode Command Line Tools installed. Additional trouble shooting for knitting to PDF can be found here: 
...
There is SO much more that you can do when building RMarkdown files, including formatting tables, embedding citations, automating report generation etc. People use RMarkdown to write textbooks, such as the 2017 version of Hadley Wickham's textbook suggested for this class: <https://r4ds.had.co.nz/>. You can go to this textbook and view the source code behind the book to see all of the `.Rmd` files used to build the book. People also use RMarkdown to write theses and dissertations, reports, and so on. There are resources listed below that can help you learn some of the extended functionality. However, for this course, the basic commands for text formatting, inserting code and output and rendering .html or .pdf documents is sufficient.
